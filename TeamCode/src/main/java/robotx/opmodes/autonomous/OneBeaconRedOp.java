 ...
 */
@Autonomous(name = "OneBeaconRedOp", group = "Autonomous")
@Disabled
public class OneBeaconRedOp extends LinearOpMode {
	...
	public void runOpMode() {
		...
		// 2. Turn left towards beacon.
		if (opModeIsActive()) {
			telemetry.addData("Stage", "2. Turn left towards beacon.");
			this.updateTelemetry(telemetry);
			tileRunnerDrive.setRightPower(0.4);
			timer.reset();
		...
		// 8. Press red beacon button.
		if (opModeIsActive()) {
			telemetry.addData("Stage", "8. Press red beacon button.");
			this.updateTelemetry(telemetry);
		...
		if (opModeIsActive()) {
			...
			this.updateTelemetry(telemetry);
			tileRunnerDrive.setRightPower(-0.4);
			tileRunnerDrive.setLeftPower(-0.1);
			timer.reset();
			while (timer.seconds() <= 1.0) {
