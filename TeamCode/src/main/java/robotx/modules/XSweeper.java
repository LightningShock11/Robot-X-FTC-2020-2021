...
public class XSweeper extends XModule {
    ...
    public void rotateSweeper(){
        if (sweeperIsOut){
            ...
            sweeperRotate.setPosition(0.95);
            sweeperIsOut = true;
        }
    }
    public void stepDown(){
        sweeperRotate.setPosition(sweeperRotate.getPosition() + .05);
    }
    public void stepUp(){
        sweeperRotate.setPosition(sweeperRotate.getPosition() - .05);
    }
    ...
    public void loop(){
        ...
        float yStickSweep = xGamepad2().right_stick_y;
        if (yStickSweep < 0){
            stepUp();
        }
        else if (yStickSweep > 0){
            stepDown();
        }
    }
}
