 ...
 */
@Autonomous(name = "NewTwoRedBeaconOp", group = "Autonomous")
@Disabled
public class NewTwoRedBeaconOp extends XLinearOpMode {
	...
	public void runOpMode() {
		...
		while ( (System.currentTimeMillis()-startLaunchingTime)<5000 && twoWheelLauncher.getNumberLaunched()<2 && !Thread.currentThread().isInterrupted() ) {
		...
		movement.pointTurnLeft(56);

		movement.driveBackward(1.0, 80);

		tileRunnerDrive.setBothPower(-0.15);
		while(floorSensor.green() <= 7 && !Thread.currentThread().isInterrupted()) {
		...
		lineFollower.followFromLeftAuton();
